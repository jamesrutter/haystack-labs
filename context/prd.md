...
### Core Stack
- **Frontend Framework**: Astro (static generation with islands architecture)
- **Content Management**: Directus (headless CMS with sophisticated relationships)
- **Hosting Platform**: Cloudflare Workers (global edge deployment)
- **Media Storage**: Cloudflare R2 (images, videos, CAD files, code)
- **Database**: PostgreSQL (via Directus)
- **Search Engine**: TBD (Algolia, MeiliSearch, or Typesense)
